...
#import "NTYSocketIOTransportXHR.h"
...
- (void) onConnect:(NTYSocketIOPacket *)packet;
- (void) onDisconnect:(NSError *)error;

- (void) sendDisconnect;
- (void) sendHearbeat;
- (void) send:(NTYSocketIOPacket *)packet;

- (NSString *) addAcknowledge:(NTYSocketIOCallback)function;
...
{
    NTYSocketIOPacket *packet = [[NTYSocketIOPacket alloc] initWithType:@"message"];
    packet.data = data;
...
{
    NTYSocketIOPacket *packet = [[NTYSocketIOPacket alloc] initWithType:@"json"];
    packet.data = [NTYSocketIOJSONSerialization JSONStringFromObject:data error:nil];
    packet.pId = [self addAcknowledge:function];
    [self send:packet];
}
...
- (void) sendEvent:(NSString *)eventName withData:(id)data andAcknowledge:(NTYSocketIOCallback)function
{
    ...
    NTYSocketIOPacket *packet = [[NTYSocketIOPacket alloc] initWithType:@"event"];
    packet.data = [NTYSocketIOJSONSerialization JSONStringFromObject:dict error:nil];
    packet.pId = [self addAcknowledge:function];
    if (function) {
        packet.ack = @"data";
    }
...
{
    NTYSocketIOPacket *packet = [[NTYSocketIOPacket alloc] initWithType:@"ack"];
    packet.data = [NTYSocketIOJSONSerialization JSONStringFromObject:data error:nil];
    packet.pId = pId;
    packet.ack = @"data";
...
- (void) sendDisconnect
{
    NTYSocketIOPacket *packet = [[NTYSocketIOPacket alloc] initWithType:@"disconnect"];
    [self send:packet];
}

- (void) sendConnect
{
    NTYSocketIOPacket *packet = [[NTYSocketIOPacket alloc] initWithType:@"connect"];
    [self send:packet];
}

- (void) sendHeartbeat
{
    NTYSocketIOPacket *packet = [[NTYSocketIOPacket alloc] initWithType:@"heartbeat"];
    [self send:packet];
}
...
- (void) doQueue 
{
    ...
    while ([_queue count] > 0) {
        NTYSocketIOPacket *packet = [_queue objectAtIndex:0];
        [self send:packet];
...
- (void) onTimeout 
{
    ...
    [self onDisconnect:[NSError errorWithDomain:NTYSocketIOError
...
- (void) onData:(NSString *)data
{
    ...
    if ([test count] > 0) {
        ...
        NTYSocketIOPacket *packet = [[NTYSocketIOPacket alloc] initWithTypeIndex:idx];
            ...
            case 6: {
                ...
                if ([pieces count] > 0) {
                    ...
                    if (argsStr && ![argsStr isEqualToString:@""]) {
                        argsData = [NTYSocketIOJSONSerialization objectFromJSONData:[argsStr dataUsingEncoding:NSUTF8StringEncoding] error:nil];
                        if ([argsData count] > 0) {
                    ...
                    NTYSocketIOCallback callbackFunction = [_acks objectForKey:key];
...
- (void) connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response 
{
    ...
    if ([response respondsToSelector:@selector(statusCode)]) {
        ...
        if (statusCode >= 400) {
            ...
            NSError *statusError = [NSError errorWithDomain:NTYSocketIOError
                                                       code:statusCode
                                                   userInfo:errorInfo];
...
- (void) connection:(NSURLConnection *)connection didFailWithError:(NSError *)error
{
    ...
    if ([_delegate respondsToSelector:@selector(socketIO:onError:)]) {
        ...
        NSError *err = [NSError errorWithDomain:NTYSocketIOError
...
{ 	
    ...
    if ([_sid length] < 1 || [data count] < 4) {
        ...
        if ([transports indexOfObject:@"websocket"] != NSNotFound) {
            ...
            _transport = [[NTYSocketIOTransportWebsocket alloc] initWithDelegate:self];
        }
        else if ([transports indexOfObject:@"xhr-polling"] != NSNotFound) {
            DEBUGLOG(@"xhr polling supported -> using it now");
            _transport = [[NTYSocketIOTransportXHR alloc] initWithDelegate:self];
        }
        else {
            DEBUGLOG(@"no transport found that is supported :( -> fail");
            connectionFailed = true;
            error = [NSError errorWithDomain:NTYSocketIOError
